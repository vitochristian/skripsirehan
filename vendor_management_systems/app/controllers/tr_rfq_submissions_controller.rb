class TrRfqSubmissionsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @tr_rfq_submission.save
        format.html { redirect_to @tr_rfq_submission, notice: 'Tr rfq submission was successfully created.' }
        format.json { render :show, status: :created, location: @tr_rfq_submission }
      else
  ...
  private
    ...
    def tr_rfq_submission_params
      params.require(:tr_rfq_submission).permit(:bidding_types, :description, :need_by_date, :term_of_payment, :no_quotation, :incoterms, :note, :is_active, :created_by, :updated_by, :status)
    end
end
